...
function return_git_commit_hash(callback) {
  ...
  gitproc.on('error', function (code) {
    ...
    if (code != 0) callback(true, undefined);
  });
      ...
      case 'callback':
        arg_array.push(function (err, payload) {
          ...
          if (!err) {
            ...
            retval.push(
              '[{0}] Error executing "{1}" because server condition not met: {2}'.format(
...
function retrieve_property(args, callback) {
  ...
  arg_array.push(function (err, payload) {
    if (!err && payload !== undefined) {
      ...
      retval.push('[{0}] Error querying property "{1}"'.format(args.options.server_name, property, err));
    }
    callback(err ? 1 : 0, retval);
  });
...
if ('version' in opt.options) {
  ...
  if (opt.argv[0] in instance) {
    ...
    handle_server(opt, function (code, retval) {
      for (var idx in retval) console.log(retval[idx]);
      process.exit(code);
    });
  } else {
