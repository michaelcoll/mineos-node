...
var localAuth = function (username, password) {
  ...
  auth.authenticate_shadow(username, password, function (authed_user) {
    if (authed_user) deferred.resolve({ username: authed_user });
    else deferred.reject(new Error('incorrect password'));
  });
...
mineos.dependencies(function (err, binaries) {
  ...
  var config_locs = ['custom.conf', '/etc/mineos.conf', '/usr/local/etc/mineos.conf'];
