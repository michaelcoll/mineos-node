...
var proc_paths = ['/usr/compat/linux/proc', '/system/lxproc', '/proc', '/compat/linux/proc'];
...
mineos.mc = function (server_name, base_dir) {
    ...
    async.waterfall(
      [
        ...
        function (stat_data, cb) {
          if (fn in memoize_timestamps && memoize_timestamps[fn] - stat_data.mtime == 0) {
  ...
  self.overlay_sp = function (dict, callback) {
    ...
    self.sp(function (err, props) {
      for (var key in dict) props[key] = dict[key];
  ...
  self.create_from_archive = function (owner, filepath, callback) {
    ...
    else var dest_filepath = path.join(self.env.base_dir, mineos.DIRS['import'], filepath);
    ...
    switch (extension) {
      ...
      case 'tar':
        ...
        var params = { cwd: self.env.cwd, uid: owner.uid, gid: owner.gid };
      ...
      async.series(
        {
          ...
          jarfile: function (cb) {
            self.sc(function (err, dict) {
              ...
              if (!jarfile) cb('Server not assigned a runnable jar');
        ...
        function (err, results) {
          if (err) {
            ...
            if (results.xmx > 0) args.push('-Xmx{0}M'.format(results.xmx));
            if (results.xms > 0) args.push('-Xms{0}M'.format(results.xms));

            if (results.java_tweaks) {
              var splits = results.java_tweaks.split(/ /);
              for (var i in splits) args.push(splits[i]);
            }
        ...
        function (err, results) {
          if (err) {
            ...
            args.push.apply(args, [
              results.binary,
              '-server',
              '-Xmx{0}M'.format(results.xmx),
              '-Xms{0}M'.format(results.xms),
            ]);
            ...
            if (results.jarfile.toLowerCase().indexOf('forge') == 0)
              if (results.jarfile.slice(-13).toLowerCase() == 'installer.jar') args.push('--installServer');
      ...
      async.series(
        {
          ...
          pharfile: function (cb) {
            self.sc(function (err, dict) {
              ...
              if (!pharfile) cb('Server not assigned a runnable phar');
        ...
        function (sc_data, cb) {
          if ((sc_data.minecraft || {}).profile) {
            self.profile_delta(sc_data.minecraft.profile, function (err, changed_files) {
              ...
              } else if (changed_files) self.copy_profile(cb);
              else cb();
            });
          } else {
            cb();
        ...
        function (cb) {
          async.whilst(
            function () {
              ...
              else return self.server_name in mineos.server_pids_up();
            },
            function (cc) {
              iterations += 1;
            ...
            function (ignored_err) {
              if (self.server_name in mineos.server_pids_up()) cb(true);
  ...
  self.restart = function (callback) {
    async.series([async.apply(self.stop), async.apply(self.start)], callback);
  };

  self.stop_and_backup = function (callback) {
    async.series([async.apply(self.stop), async.apply(self.backup)], callback);
  };
  ...
  self.kill = function (callback) {
    ...
    if (!(self.server_name in pids)) {
      ...
      async.doWhilst(
        ...
        function (ignored_err) {
          if (self.server_name in mineos.server_pids_up()) callback(true);
      ...
      [
        ...
        function (cb) {
          cb(
            ...
            child_process.spawn(
              binary,
              ['-S', 'mc-{0}'.format(self.server_name), '-p', '0', '-X', 'eval', 'stuff "{0}\012"'.format(msg)],
              params
            )
          );
        },
      ...
      [
        ...
        function (cb) {
          cb(
            ...
            child_process.spawn(
              binary,
              ['-S', 'mc-{0}'.format(self.server_name), '-p', '0', '-X', 'eval', 'stuff "save-all\012"'],
              params
            )
          );
        },
  ...
  self.saveall_latest_log = function (callback) {
    ...
    async.waterfall(
      [async.apply(self.verify, 'exists'), async.apply(self.verify, 'up'), async.apply(self.stuff, 'save-all')],
      function (err) {
        if (err) {
          clearTimeout(timeout);
          new_tail.unwatch();
      ...
      case 'server_files':
        ...
        async.waterfall(
          [
            ...
            function (sf, cb) {
              server_files.push.apply(
                server_files,
                sf.filter(function (file) {
                ...
                if (err) {
                  ...
                  server_files.push.apply(
                    ...
                    files.filter(function (file) {
                      return (
                        (file.substr(-4).toLowerCase() == '.jar' && server_files.indexOf(file) < 0) ||
  ...
  self.ping = function (callback) {
    ...
    function buffer_to_ascii(buf) {
      var retval = '';
      for (var i = 0; i < buf.length; i++) retval += buf[i] == 0x0000 ? '' : String.fromCharCode(buf[i]);
      return retval;
    }
