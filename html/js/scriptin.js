...
app.filter('membership', function () {
  return function (servers) {
    ...
    for (var s in servers) if (servers[s].sp) keep[s] = servers[s];
...
app.filter('profile_filter', function () {
  return function (profiles, criteria) {
    ...
    if (criteria.value == 'all') return profiles;
    else {
      for (var index in profiles) if (criteria.value == profiles[index][criteria.field]) keep.push(profiles[index]);
...
app.filter('profile_downloaded', function () {
  return function (profiles, criteria) {
    ...
    if (criteria == 'all') return profiles;
    else {
      for (var index in profiles)
        if (criteria == 'only_downloaded' && profiles[index].downloaded) keep.push(profiles[index]);
...
app.filter('remove_old_versions', function () {
  return function (profiles, remove_older_than) {
    ...
    for (var index in profiles) {
      ...
      if (parseInt(ver[1]) > major) keep.push(profiles[index]);
      else if (parseInt(ver[1]) == major)
        if (parseInt(ver[2]) > minor) keep.push(profiles[index]);
        else if (parseInt(ver[2]) == minor) if (parseInt(ver[3]) || 0 >= patch) keep.push(profiles[index]);
    }
    ...
    */
      ...
      for (i in splitString) {
        ...
        if (i % 2 == 1) {
          ...
          if (splitString[i] == undefined) splitString[i] = spanOpen ? '</span>' : '';
          else {
            ...
            for (j in formatCodes) {
              ...
              if (formatCode == ANSI_INTENSE_CODE) intense = true;
              else if (formatCode == ANSI_NORMAL_CODE) intense = false;
              else if (formatCode >= ANSI_MIN_COLOR && formatCode <= ANSI_MAX_COLOR) colorCode = formatCode;
              else console.log('Unsupported format code: ' + formatCode);
            }

            // Add color class to span
            if (intense) splitString[i] += IntenseColors[colorCode - ANSI_COLOR_OFFSET];
            else splitString[i] += Colors[colorCode - ANSI_COLOR_OFFSET];

            // Close span off
            splitString[i] += '">';
            spanOpen = true;
          }
        } else splitString[i] = escapeChars(splitString[i]);
      }
...
app.controller('Webui', [
  '$scope',
  'socket',
  'Servers',
  '$filter',
  '$translate',
  function ($scope, socket, Servers, $filter, $translate) {
    ...
    $scope.$watch(
      ...
      function () {
        ...
        if (!($scope.current in Servers)) $scope.change_page('dashboard');
      }
    );
    ...
    $scope.players_online = function () {
      ...
      $.each(Servers, function (server_name, instance) {
        try {
          if (instance.heartbeat.ping.players_online) online += instance.heartbeat.ping.players_online;
        } catch (e) {}
      });
      return online;
    };
    ...
    $scope.unique_profile_groups = function () {
      ...
      if ($scope.profiles)
        $.each($scope.profiles, function (idx, data) {
          if (groups.indexOf(data.group) < 0) groups.push(data.group);
        });
      return groups;
    };
    ...
    $scope.extra_action = function (action) {
      ...
      var show = false;
      if (profile.startsWith('bungee')) show = EXTRA_ACTIONS['bungeecord'].indexOf(action) >= 0;
      else if (profile.startsWith('cuberite')) show = EXTRA_ACTIONS['cuberite'].indexOf(action) >= 0;
      else if (profile.startsWith('nukkit')) show = EXTRA_ACTIONS['nukkit'].indexOf(action) >= 0;
      else if (profile.startsWith('paperspigot')) show = EXTRA_ACTIONS['paperspigot'].indexOf(action) >= 0;
      else show = EXTRA_ACTIONS['default'].indexOf(action) >= 0;
    ...
    socket.on('/', 'profile_list', function (profile_data) {
      ...
      for (var p in profile_data)
        if (profile_data[p].id == 'BuildTools-latest') $scope.buildtools_jar = profile_data[p];
        else if (profile_data[p].id == 'PaperTools-latest') $scope.papertools_jar = profile_data[p];
    });
    ...
    socket.on('/', 'file_progress', function (data) {
      for (var p in $scope.profiles)
        if (
          data.group == $scope.profiles[p].group &&
    ...
    $scope.server_command = function (cmd, args) {
      ...
      } else socket.emit($scope.current, 'command', { command: cmd });
    };
    ...
    $scope.host_command = function (cmd, args) {
      ...
      } else socket.emit('/', 'command', { command: cmd });
    };

    $scope.cron_command = function (cmd, args) {
      args['operation'] = cmd;
      if (args['command'] != 'stuff') args['msg'] = '';
      ...
      if (!regex_valid_server_name.test(server_name)) {
        ...
        if ($scope.unconventional) {
          ...
          for (var prop in serverform)
            if (serverform.hasOwnProperty(prop)) hyphenated[prop.split('_').join('-')] = serverform[prop]; //replace _ with -
    ...
    $scope.server_from_archive_create = function () {
      ...
      if ($scope.awd_dir && $scope.current) obj['awd_dir'] = $scope.current;
      else obj['awd_dir'] = null;
    ...
    $scope.update_loadavg = function (new_datapoint) {
      ...
      while ($scope.loadavg.length > $scope.loadavg_options.xaxis.max) $scope.loadavg.splice(0, 1);

      function get_enumerated_values(column) {
        var res = [];
        for (var i = 0; i < $scope.loadavg.length; ++i) res.push([i, $scope.loadavg[i][column]]);
        return res;
      }

      function get_max_y(arr) {
        var max_y = 1;
        for (var i = 0; i < arr.length; i++) if (arr[i][1] > max_y) max_y = arr[i][1];
        return max_y;
      }
  ...
  function (socket, $filter) {
    ...
    var server_model = function (server_name) {
      ...
      me.auto_rate_interval = setInterval(function () {
        if (
          me.auto_rate_counter * me.AUTO_RATE_SUSTAINED_DURATION >
          me.AUTO_RATE_THRESHOLD_PER_SECOND * me.AUTO_RATE_SUSTAINED_DURATION
        )
          me.auto_rate_interval = null;
        else me.auto_rate_counter = 0;
      ...
      me.channel.on(server_name, 'server_fin', function (data) {
        ...
        if (data.command == 'archive') me.refresh_archives();

        if (data.command == 'backup') me.refresh_increments();

        if (data.command == 'prune') me.refresh_increments();
...
app.factory('socket', function ($rootScope) {
  ...
  if (port === null) {
    ...
    if (window.location.protocol == 'https:') var port = '443';
    else var port = '80';
  }
  ...
  return {
    on: function (server_name, eventName, callback) {
      if (!(server_name in sockets)) {
        if (server_name == '/') sockets[server_name] = io(connect_string, { secure: true });
        else sockets[server_name] = io(connect_string + server_name, { secure: true });
      }
