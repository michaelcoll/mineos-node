...
test.dependencies_met = function (test) {
  async.series([async.apply(mineos.dependencies)], function (err, results) {
    test.ifError(err);
    test.done();
  });
};
...
test.extract_server_name = function (test) {
  ...
  test.throws(function () {
    mineos.extract_server_name(BASE_DIR, '/home/runner/minecraft/servers');
  }, 'no server name in path');
  test.throws(function () {
    mineos.extract_server_name(BASE_DIR, '/home/runner/minecraft');
  }, 'no server name in path');
  test.done();
};
...
test.check_eula = function (test) {
  ...
  async.waterfall(
    [
      async.apply(instance.create, OWNER_CREDS),
      function (throwaway, cb) {
        cb();
      }, //discards instance.create return so cb arguments line up below
      ...
      async.apply(
        fs.outputFile,
        path.join(instance.env.cwd, 'eula.txt'),
